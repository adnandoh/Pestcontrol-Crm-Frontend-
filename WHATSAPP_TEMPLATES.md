# PestControl99 — WhatsApp Message Templates (CRM)

---

## T1 — Booking confirmation → Client

**Sent to:** Customer  
**When:** After booking is confirmed

---

Dear {Client Name},

Your service booking has been successfully confirmed.

Booking ID: {Booking ID}

Service Details:
• Selected Service Type: {Service Type}
• Selected Area: {Area}
• Service Date: {Date}
• Service Time: {Time}
• Service Amount: ₹{Amount}
• Service Address: {Address}

Our technician will contact you before arrival.

Terms & Conditions Apply:
https://www.pestcontrol99.com/terms-and-conditions/

For support or rescheduling:
📞 8080748282

Regards,
PestControl99 Team

---

## T2 — Technician assigned → Client

**Sent to:** Customer  
**When:** After a technician is assigned to the job

---

Dear {Client Name},

Your technician has been assigned for the scheduled pest control service.

Booking ID: {Booking ID}

Technician Details:
• Technician Name: {Technician Name}
• Contact Number: {Technician Mobile}

Service Details:
• Selected Service Type: {Service Type}
• Selected Area: {Area}
• Visit Date & Time: {Date & Time}
• Service Amount: ₹{Amount}

Terms & Conditions Apply:
https://www.pestcontrol99.com/terms-and-conditions/

Thank you,
PestControl99 Team

---

## T3 — Customer inquiry / rate card → Client

**Sent to:** Customer  
**When:** New inquiry or lead (Website Leads / CRM Inquiries)

---

Dear {Client Name},

Thank you for contacting us. Please find our service details below:

🪳 Cockroach / Ant Control

✔️ AMC Plan (Recommended)
• 1 Year Protection
• 3 Services (Gel + Spray)

1RK – ₹1800
1BHK – ₹2200
2BHK – ₹2500
3BHK – ₹3000
4BHK – ₹3500

✔️ One-Time Service

1RK – ₹1000
1BHK – ₹1200
2BHK – ₹1500
3BHK – ₹1800
4BHK – ₹2000

🛏️ Bed Bugs Treatment
(2 Services) ₹2200 – ₹4000

🪵 Termite Treatment
(2 Years Warranty) ₹2500 – ₹4000

✔️ Safe & No-Smell Treatment
✔️ Professional & Trained Technicians
✔️ Same Day / Next Day Service Available

Terms & Conditions: https://www.pestcontrol99.com/terms-and-conditions/
📞 7710032627

Please confirm your preferred date & time for booking.

Regards,
PestControl99 Team

---

## T4 — New service assigned → Technician

**Sent to:** Technician  
**When:** Job is assigned; full job details for field visit

---

Dear {Technician Name},

New Service Assigned

Booking ID: {Booking ID}

Client Details:
• Client Name: {Client Name}
• Mobile Number: {Client Mobile}

Service Details:
• Selected Service Type: {Service Type}
• Selected Area: {Area}
• Service Amount: ₹{Amount}
• Service Address: {Address}
• Booking Date & Time: {Date & Time}

Special Instructions:
{Notes or N/A}

Terms & Conditions: https://www.pestcontrol99.com/terms-and-conditions/

Please contact the client before reaching the location.

Regards,
PestControl99 Operations Team

---

*If technician name is not set, the greeting “Dear {Technician Name},” is omitted and the message starts with “New Service Assigned”.*
