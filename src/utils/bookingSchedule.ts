...
function visitTypeLabel(service: string, plan: string, cycle = 1): string {
  ...
  if (isBedBugService(service)) return 'BED BUG SERVICE';
...
function visitDateForCycle(
  start: dayjs.Dayjs,
  cycleIndex: number,
  service: string,
  visitCount: number,
): dayjs.Dayjs {
  const { unit, value } = getAmcIntervalSpec(service, visitCount);
  if (unit === 'day') {
    return start.add(value * cycleIndex, 'day');
  }
  return start.add(value * cycleIndex, 'month');
}
...
function buildVisitDates(
  ...
  // Product rule: Bed Bugs is always 2 services (day 0 + ~15 days), even though
  // the stored plan value is "One Time Service".
  if (isBedBugService(service)) {
    return [
      { visitNumber: 1, date: start, label: 'BED BUG SERVICE' },
      { visitNumber: 2, date: start.add(15, 'day'), label: 'BED BUG SERVICE' },
    ];
  }
  ...
  return Array.from({ length: count }, (_, i) => ({
    visitNumber: i + 1,
    date: visitDateForCycle(start, i, service, count),
...
export function previewServiceSchedule(
  ...
  const count = parseAmcCountFromPlan(plan);
  const intervalHint = isBedBugService(service)
    ? '2nd visit after 15 days'
    : count
      ? AMC_INTERVAL_LABELS[count] || null
      : null;

  if (!scheduleDate?.trim()) {
    const totalVisits = isTermiteService(service)
      ? 5
