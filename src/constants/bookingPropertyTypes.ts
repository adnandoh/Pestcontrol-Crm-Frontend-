...
export function isSocietyBooking(input: {
  commercial_type?: string;
  property_type?: string;
  job_type?: string;
}): boolean {
  return (
    input.commercial_type === 'society' ||
    input.property_type === 'Society' ||
    input.job_type === 'Society'
  );
}

/** Backend society renewals use 12 / 6 / 3 month contract_duration values. */
export function deriveSocietyContractDuration(
  serviceItems?: { plan?: string }[],
): '12' | '6' | '3' {
  const plans = (serviceItems || [])
    .map((item) => String(item.plan || ''))
    .join(' ')
    .toLowerCase();
  if (plans.includes('amc 3')) return '3';
  if (plans.includes('amc 6')) return '6';
  return '12';
}

/** Human-readable AMC interval labels. */
