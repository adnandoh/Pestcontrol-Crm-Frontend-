      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 animate-fade-up delay-200">
        <section className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <span className="rounded-lg bg-indigo-600 px-2.5 py-1 text-[10px] font-black text-white">
              {stats?.today_booking_count ?? 0} bookings
            </span>
          </div>
        ...
        <section className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-[11px] font-black uppercase tracking-widest text-gray-900">
                  Today’s Service Calls by City
                </h2>
                <p className="text-[10px] font-medium text-gray-500">
                  AMC follow-ups & service visits today
                </p>
              </div>
            </div>
            <span className="rounded-lg bg-emerald-600 px-2.5 py-1 text-[10px] font-black text-white">
              {stats?.today_service_call_count ?? 0} service
            </span>
          </div>
          {(stats?.today_service_city_stats?.length || 0) === 0 ? (
            <p className="text-xs font-medium text-gray-400">No service calls scheduled for today yet.</p>
          ) : (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {(stats?.today_service_city_stats || []).map((row) => (
                <div
                  key={`svc-${row.city}`}
